...
/**
 * <code>Backup</code> implements a solver for
 * <code>Maze</code> objects using a fork/join multi-thread
 * depth-first search.
 * <p>
 * Instances of <code>Backup</code> should be run by a
...
{
  private static ConcurrentSkipListSet<Integer> concurrentVisited;
  ...
  private static ConcurrentHashMap<Integer, Integer> concurrentPredecessor;
  private int stepsSinceFork;
     ...
     */
    public Backup(Maze maze)
    {
        super(maze);
        concurrentVisited = new ConcurrentSkipListSet<Integer>();
        concurrentPredecessor = new ConcurrentHashMap<Integer, Integer>();
        stepsSinceFork=0;
    }
     ...
     *                    forks new tasks
     */
    public Backup(Maze maze, int forkAfter)
    {
        super(maze);
        concurrentVisited = new ConcurrentSkipListSet<Integer>();
        concurrentPredecessor = new ConcurrentHashMap<Integer, Integer>();
      ...
      while (!frontier.empty()) {
          ...
          if (!concurrentVisited.contains(current)) {
              ...
              if(stepsSinceFork>=forkAfter){
                Backup[] forkedChildren = new Backup[maze.neighbors(current).size()];
                int child=0;
                for (int nb: maze.neighbors(current)) {
                  // add nb to the nodes to be processed
                  forkedChildren[child]=new Backup(maze, forkAfter);
